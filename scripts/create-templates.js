// 36개 다양한 레이아웃 템플릿 생성 스크립트
const templates = [];

// 1장 템플릿 (6개)
const onePhotoTemplates = [
  // 세로형
  {
    name: "1장 세로형 - 전체 화면",
    photoCount: 1,
...
// 모든 템플릿 합치기
templates.push(...onePhotoTemplates);
templates.push(...twoPhotoTemplates);
templates.push(...threePhotoTemplates);
templates.push(...fourPhotoTemplates);
templates.push(...fivePhotoTemplates);
templates.push(...sixPhotoTemplates);

// 템플릿 삽입 함수
async function insertTemplates() {
  const baseUrl = 'http://localhost:3001';
  
  console.log(`총 ${templates.length}개의 템플릿을 생성합니다...`);
  ...
  for (let i = 0; i < templates.length; i++) {
    ...
    try {
      ...
      if (response.ok) {
        console.log(`✓ ${i + 1}/${templates.length}: ${template.name}`);
      } else {
        console.error(`✗ ${i + 1}/${templates.length}: ${template.name} - ${response.status}`);
      }
    } catch (error) {
      console.error(`✗ ${i + 1}/${templates.length}: ${template.name} - ${error.message}`);
    }
  }
...
if (typeof window !== 'undefined') {
  window.insertTemplates = insertTemplates;
  window.templates = templates;
}

module.exports = { templates, insertTemplates };